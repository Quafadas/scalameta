...
package object scalameta {
  ...
  inline def unreachable: Nothing = UnreachableMacros.unreachable
  ...
  //   baz = 42
  //   foo + bar = "foobar"
  inline def unreachable(dsl: Boolean): Nothing = UnreachableMacros.unreachableWithDebug(dsl)

  inline def unreachable(dsl: Boolean, clue: String): Nothing =
    UnreachableMacros.unreachableWithDebugAndClue(dsl, clue)
